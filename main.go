...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "ada2d960.pmuir",
	})
	if err != nil {
	...
	if err = (&kafka.KafkaInstanceReconciler{
	...
	if err = (&kafka.KafkaInstancePeriodicReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "KafkaInstancePeriodic")
		os.Exit(1)
	}
	...
	if err = (&serviceaccount.ServiceAccountReconciler{
		...
		setupLog.Error(err, "unable to create controller", "controller", "CloudServiceAccount")
		os.Exit(1)
	}
	if err = (&serviceaccount.ServiceAccountPeriodicReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
